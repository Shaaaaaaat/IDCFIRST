...
function getPriceAndSchedule(studio) {
  const priceSchedule = {
    ...
    "ул. Бузанда":
      "Адрес студии на ул. Бузанда.:\nУл. Павстоса Бузанда, 1/3\n\n🔻 Расписание занятий:\nПонедельник 08:30 (утро) \nСреда 08:30 (утро) \nПятница 08:30 (утро) \n\n🔻 Стоимость тренировок:\n👉🏻Пробное - 5000 AMD (действует 4 недели)\n👉🏻12 занятий - 60000 AMD (действует 6 недель)\n👉🏻1 занятие - 7000 AMD (действует 4 недели)\n\n🔻 Цены индивидуальных тренировок:\n1 тренировка (1 чел.) - 12500 AMD за занятие\n1 тренировка (2 чел.) - 17000 AMD за занятие\n1 тренировка (3 чел.) - 21000 AMD за занятие",
  };
...
bot.on("message:text", async (ctx) => {
  ...
  if (session.step === "awaiting_personal_training_details") {
    ...
    // Извлекаем данные о теге из информации пользователя
    const userInfo = await getUserInfo(ctx.from.id);

    if (userInfo) {
      const { tag } = userInfo;

      // Генерация клавиатуры для оплаты на основе тега
      const keyboard = generateKeyboard(tag);

      if (keyboard) {
        // Отправляем кнопки для выбора тарифа после основного сообщения
        await ctx.reply("Выберите подходящий тариф для оплаты:", {
          reply_markup: keyboard,
        });
      } else {
        // Если тег не найден, сообщаем пользователю
        await ctx.reply(
          "Ваш тег не распознан, и мы не можем предложить соответствующие тарифы. Пожалуйста, обратитесь к поддержке."
        );
      }
    } else {
      await ctx.reply(
